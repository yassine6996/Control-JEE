...
public class HospitalApplication  {
    ...
    CommandLineRunner start(IHospitalService hospitalService,
        ...
        return args -> {
            Stream.of("yassine","ismail","amine")
                    .forEach(name ->{
            ...
            Stream.of("walid","aya","fahd")
                    .forEach(name ->{
                        Medecin medecin = new Medecin();
                        medecin.setNom(name);
                        medecin.setSpecialite(Math.random()>0.5?"gero":"Dentiste");
                        medecin.setEmail(name+"@gmail.com");
                        hospitalService.saveMedecin(medecin);
                    });

            Patient patient = patientRepository.findById(1L).orElse(null);
            Patient patient1 = patientRepository.findByNom("ismail");

            Medecin medecin = medecinRepository.findByNom("walid");
